...
export default function CompanyCommonFacilityEdit() {
    
    const {currentCompany} = useContext(AuthContext);
    const [commonFaility, setCommonFacility] = useState();
    ...
    const commonFacilityId = location.pathname.split('/')[3];

    
    
    useEffect(()=>{
        const fetchCommonFacility = async () => {
                   
            try {
                const res = await axios.get(`/common-facilities/${commonFacilityId}`)

                setCommonFacility(res.data);
    ...
    const submit = async (e) => {
        ...
        try {
            await axios.put(`/common-facilities/${commonFaility.id}`, inputs);
                ...
                <div className="bg-white rounded-lg overflow-hidden">
                    <div className="relative">
                        <div className="flex gap-2 items-center bg-slate-200 rounded-t px-4 py-2">
                            <div className="py-2">
                                <h4 className="font-bold text-color-main leading-4 text-lg uppercase">Edit Common Facility</h4>
                            </div>
                        </div>
                    </div>
                    {isProcessing ? 

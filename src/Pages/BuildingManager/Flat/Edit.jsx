...
export default function BuildingManagerFlatEdit() {
    ...
    const flatId = location.pathname.split('/')[3];
    ...
    useEffect(()=>{
        const fetchFlat = async () => {
            ...
            try {
                const res = await axios.get(`/flats/${flatId}`)

                setInputs({
                    building_id: res.data.building_id || '',
                    name: res.data.name || '', 
                    flat_no: res.data.flat_no || '',
                    flat_category_id: res.data.flat_category_id || '',
                });
            } 
            catch (error){
            } 
    ...
    const submit = async (e) => {
        ...
        try {
            await axios.put(`http://localhost:8800/api/flats/${flatId}`, inputs);
                ...
                <div className="bg-white rounded-lg overflow-hidden">
                    <div className="relative">
                        <div className="flex gap-2 items-center bg-slate-200 rounded-t px-4 py-2">
                            <div className="py-2">
                                <h4 className="font-bold text-color-main leading-4 text-lg uppercase">Edit a Flat</h4>
                            </div>
                        </div>
                    </div>
                    {isProcessing ? 
